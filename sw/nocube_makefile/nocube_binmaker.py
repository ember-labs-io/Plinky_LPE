...
def main():
    # bin file maker
    bootloader_file = '../../builds/golden_bootloader.bin' # "bootloader/Release/plinkybl.bin"
    try:
        with open(bootloader_file, "rb") as f1:
            bl_content = f1.read()
        with open("RELEASE/plinkyblack.bin", "rb") as f2:
            app_content = f2.read()
    except IOError as e:
    ...
    fname = f"../../builds/{FIRMWARE_PREFIX}-{ver_major}.{ver_minor}.{ver_patch}.bin"
    ...
    try:
        ...
        exit(2)
    fname = f"../../builds/{FIRMWARE_PREFIX}-{ver_major}.{ver_minor}.{ver_patch}.uf2"
    print(f'outputting {fname}...')
    ...
    uf2args.input='RELEASE/plinkyblack.hex'
    ...
    with open(f"../../builds/BOOTLOAD.UF2", "wb") as f1:
    ...
    print("wrote a combiled firmware and bootloader package to ../../builds/BOOTLOAD.UF2")
    checksum=0
    ...
    # open ../Core/Src/plinky/plinky.h and find the GOLDEN_CHECKSUM line
    with open("../Core/Src/plinky/plinky.h", "r") as f:
        for line in f:
